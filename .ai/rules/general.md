...
**Every `<file>` exclusion states its reason and what removes it.** An exclusion for a deleted file does nothing and the gate still says pass — two dead entries (`Member/MembershipController.php`, `Quote/QuoteController.php`) survived here for months before being removed in #10. A test will enforce both directions — the path must exist, and it must carry a reason (#11, not yet written).
